    ...
    <div style={{
      ...
      {products.map(p => (
        ...
        onMouseLeave={e => {
          ...
          {/* Görsel */}
          <a href={`/urunler/${p.slug}`} style={{
            display:'flex',alignItems:'center',justifyContent:'center',
            background:'var(--cream-deep)',padding:'36px',
            textDecoration:'none',position:'relative',aspectRatio:'4/3',
          }}>
            {p.badge && (
              <span style={{
          ...
          <div style={{padding:'20px',flex:1,display:'flex',flexDirection:'column',gap:'8px'}}>
            <p style={{fontSize:'.72rem',color:'var(--ink-faint)',fontFamily:'var(--font-mark)',
              letterSpacing:'.1em',margin:0}}>{p.category}</p>
            <a href={`/urunler/${p.slug}`} style={{
            ...
            {p.stock < 50 && (
              <div>
                <div style={{height:'4px',background:'var(--cream-deep)',borderRadius:'2px',
                  overflow:'hidden',marginBottom:'4px'}}>
                  <div style={{height:'100%',width:`${Math.min(p.stock*2,100)}%`,
                    background:'var(--rust)',borderRadius:'2px'}}/>
                </div>
                <p style={{fontSize:'.72rem',color:'var(--rust)',fontWeight:600,margin:0}}>
                  Son {p.stock} adet
                </p>
              </div>
            )}
            ...
            <div style={{display:'flex',gap:'10px',marginTop:'8px',alignItems:'center'}}>
              <span style={{fontFamily:'var(--font-display)',fontSize:'1.2rem',
                fontWeight:700,color:'var(--ink)',flex:1}}>{p.priceDisplay}</span>
